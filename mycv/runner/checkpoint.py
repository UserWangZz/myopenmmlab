...
ENV_MYCV_HOME = 'MYCV_HOME'
ENV_XDG_CACHE_HOME = 'XDG_CACHE_HOME'
DEFAULT_CACHE_DIR = '~/.cache'


def _get_mycv_home() -> str:
    mycv_home = os.path.expanduser(
        os.getenv(
            ENV_MYCV_HOME,
            os.path.join(
                os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), 'mycv')))

    mkdir_or_exist(mycv_home)
    return mycv_home
...
def get_torchvision_models():
    if digit_version(torchvision.__version__) < digit_version('0.13.0a0'):
        ...
        json_path = osp.join(mycv.__path__[0],
                             'model_zoo/torchvision_0.12.json')
        model_urls = mycv.load(json_path)
...
def get_external_models():
    mycv_home = _get_mycv_home()
    default_json_path = osp.join(mycv.__path__[0], 'model_zoo/open_mmlab.json')
    default_urls = load_file(default_json_path)
    assert isinstance(default_urls, dict)
    external_json_path = osp.join(mycv_home, 'open_mmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(mycv.__path__[0], 'model_zoo/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(mycv.__path__[0],
...
def load_from_ceph(filename: str,
    ...
    .. warning::
        :class:`mycv.fileio.file_client.CephBackend` will be deprecated,
        please use :class:`mycv.fileio.file_client.PetrelBackend` instead.
...
def load_from_openmmlab(
    ...
    if model_url.startswith(('http://', 'https://')):
        ...
        filename = osp.join(_get_mycv_home(), model_url)
        if not osp.isfile(filename):
            raise FileNotFoundError(f'{filename} can not be found.')
        checkpoint = torch.load(filename, map_location=map_location)
    return checkpoint
...
def save_checkpoint(model: torch.nn.Module,
    ...
    Args:
        ...
        file_client_args (dict, optional): Arguments to instantiate a
            FileClient. See :class:`mycv.fileio.FileClient` for details.
            Default: None.
    ...
    meta.update(mycv_version=mycv.__version__, time=time.asctime())
